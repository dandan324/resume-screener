...
class ResumeScoringEngine:
    ...
    def score(self, text, name="未知"):
        ...
        return {
            ...
            'dims': {
                '岗位匹配度': {'s': round(jm,1), 'w':'35%', 'r': jmr},
                '经验深度': {'s': round(ex,1), 'w':'25%', 'r': exr},
                '稳定性': {'s': round(st,1), 'w':'20%', 'r': strs},
                '成长潜力': {'s': round(po,1), 'w':'20%', 'r': por}
            },
            'risks': risks, 'mq': mq, 'bq': bq,
            'rec': {'hd': hd, 'sal': f"{sb}元/月（试用期）" if sb>0 else "—", 'pos': pos,
                   'fp': ["前3个月观察稳定性（出勤、请假、离职倾向）","验证文创零售适应度（主动推销、审美、学习意愿）"] if hd!="建议淘汰" else [],
                   'rr': "；".join([r['desc'] for r in hr]) if hr else "", 'alt': "暂不适合" if hd=="建议淘汰" else "当前岗位已匹配"},
            'info': {'age': age, 'edu': edu, 'wc': len(we), 'tm': sum(e['dm'] for e in we)}
        }

# UI
st.markdown('<h1 style="text-align:center; color:#1f1f1f; border-bottom:3px solid #4472C4; padding:20px;">🧠 脑子进水 · 文创潮品门店营业员简历智能筛选</h1>', unsafe_allow_html=True)
        ...
        if not text.strip():
            ...
            with c2:
                ...
                <div style="background: {color}; padding: 25px; border-radius: 15px; text-align: center; color: white; margin-bottom: 20px;">
                    ...
                    <div style="font-size: 56px; font-weight: bold; margin: 10px 0;">{sc}/100</div>
                    <div style="display: inline-block; background: rgba(255,255,255,0.2); padding: 8px 20px; border-radius: 20px; font-size: 16px; font-weight: bold;">
                        {gr} · {r['decision']}
                    </div>
                </div>
                ...
                for dn, dim in r['dims'].items():
                    ...
                    st.markdown(f"""
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 10px; border-left: 4px solid {bc}; margin: 10px 0;">
                        <div style="display: flex; justify-content: space-between;">
                            <span style="font-weight: bold;">{dn}</span>
                            <span style="color: {bc}; font-weight: bold; font-size: 18px;">{dim['s']}/100</span>
                        </div>
                        <div style="background: #e5e7eb; border-radius: 10px; height: 8px; margin-top: 8px;">
                            <div style="background: {bc}; width: {p*100}%; height: 8px; border-radius: 10px;"></div>
                        </div>
